...
if(isset($_POST['submit']))
  {
    ...
    $email=$_POST['mail'];
        ...
        $query=mysqli_query($con,"select m_id from member where  m_mail='$email' or m_mob='$contactno' ");
    $ret=mysqli_fetch_array($query);
    if($ret>0){
      $_SESSION['contactno']=$contactno;
      $_SESSION['email']=$email;
     header('location:restpasswordshop.php');
    }
    else{
        echo "<script>alert('No Record Fond Accociated With This Information.')</script>";
        echo '<script>window.location="shoppassword.php"</script>';
    }
  }
  ?>
<!DOCTYPE html>
