...
$time=date($time);
//die($time);
 $query=mysqli_query($con,"insert into schedule_visitor(v_id,e_id,reason,date,time) values('$vid','$employee','$res','$date','$time')");
 if ($query) {
    //$msg="Visitor Scheduled.";
   $q1="SELECT * FROM `schedule_visitor` WHERE  `v_id` LIKE '$vid' and date='$date' and time='$time'";
    $res=mysqli_query($con,$q1);
    while($row=mysqli_fetch_assoc($res)){
        $id=$row["sch_vi_id"];
        $query1=mysqli_query($con,"insert into visit_record(sch_v_id,status) values ('$id','1')");
    }
